...
La app lee la URL base del backend con `--dart-define` (key: `OWNER_BACKEND_BASE_URL`).

```bash
flutter run --dart-define=OWNER_BACKEND_BASE_URL=http://localhost:4000
```
...
- `flutter run --dart-define=OWNER_BACKEND_BASE_URL=...`
- `flutter build apk --dart-define=OWNER_BACKEND_BASE_URL=...`

Ejemplo (producción):
- `flutter build apk --dart-define=OWNER_BACKEND_BASE_URL=https://fullpos-backend-fullpos-backend.onqyr1.easypanel.host`
